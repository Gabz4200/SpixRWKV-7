...
class SpatialMixer(nn.Module):
    ...
    def forward(
        ...
        out_fwd, vf_fwd = self.scan(xn, xx, dm, "forward", v_first_fwd)
        out_bwd, vf_bwd = self.scan(xn, xx, dm, "backward", v_first_bwd)
...
class Vision_RWKV7_Block(nn.Module):
    ...
    def __init__(
        ...
        self.channel_mix = ChannelMix(n_embd, drop_prob=drop_prob, init_values=init_values)
...
class SuperpixelTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = SuperpixelEmbedding(
            in_chans, embed_dims, num_superpixels, mode=mode,
        )
...
class Vision_RWKV7(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, num_superpixels: Optional[int] = None
    ):
        B, C, H, W = x.shape
        ...
        # ---- Tokenization ----
        out = self.tokenizer(x, num_superpixels=num_superpixels, spixel_size=self.spixel_size)
        tokens = out["tokens"]
