...
def lbl_to_rgb(
    lbl: np.ndarray, color_palette: Optional[np.ndarray] = None
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        img = (
            torch.tensor(face()).permute(2, 0, 1).unsqueeze(0).contiguous().float()
            / 255
            * 2
            - 1
        )
        ...
        coords = torch.stack(
            torch.meshgrid(
                torch.linspace(-1, 1, h, dtype=torch.float),
                torch.linspace(-1, 1, w, dtype=torch.float),
            ),
            -1,
        ).unsqueeze(0)
        # sin embedding
        ...
        scaled_x = (coords[..., None, :] * freqs[..., None]).reshape(
            shape
        )  # (batch, *, n_points, num_feats * n_freq)
        scaled_x = torch.stack([scaled_x, scaled_x + 0.5 * torch.pi], -2).reshape(
            shape
        )  # (batch, n_points, 2 * num_feats * n_freq)
        ...
        assert assign.shape[-2:] == inputs.shape[-2:], (
            f"{assign.shape[-2:]} v.s., {inputs.shape[-2:]}"
        )
        # assignment to label
        h_s, w_s = feats.shape[-2:]
        hard_assign = (
            F.one_hot(assign.argmax(1), (2 * args.candidate_radius + 1) ** 2)
            .permute(0, 3, 1, 2)
            .contiguous()
            .float()
        )
        label = torch.arange(h_s * w_s, dtype=torch.float).reshape(1, 1, h_s, w_s)
        label = spixel_upsampling(
            label, hard_assign, candidate_radius=args.candidate_radius
        )
