...
import torch
from VisualRWKV7.diffSLIC import DiffSLIC
...
def test_diffslic_all_black_no_nan():
    ...
    slic = DiffSLIC(n_spixels=8, n_iter=3, tau=0.01, candidate_radius=1, normalize=True, stable=True)
    with torch.no_grad():
...
def test_model_all_black_finite():
    ...
    model = Vision_RWKV7(img_size=32, embed_dims=64, num_heads=1, depth=1, num_superpixels=9, diff_slic_iters=2)
    outs = model(x)
    for o in outs:
...
def test_model_extreme_hyperparams():
    ...
    for n_spx, iters in configs:
        model = Vision_RWKV7(img_size=32, embed_dims=64, num_heads=1, depth=1, num_superpixels=n_spx, diff_slic_iters=iters)
...
def test_model_batch_consistency():
    ...
    assert torch.allclose(out_batch[0][:1], out_single[0], atol=1e-4), "Batch and single outputs differ"
    assert torch.allclose(out_batch[0][0], out_batch[0][1], atol=1e-4), "Batch items differ"
