...
def build_rwkv(
    ...
    if model_type == "hybrid":
        eff_df = float(downsample_factor) if downsample_factor is not None else config.get("downsample_factor", 16)
...
def run_resolution_sweep(
    ...
    config_path = config_dir / (f"gnn_{size}.yaml" if model_type == "gnn" else f"conv_{size}.yaml" if model_type == "conv" else f"vq_{size}.yaml" if model_type == "vq" else f"hybrid_{size}.yaml" if model_type == "hybrid" else f"{size}.yaml")
