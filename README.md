...
> **DISCLAIMER 1:** This repository is a learning project made by a single Brazilian student that is exploring the design space of RWKV-based vision backbones. The architecture started as a port from VisionRWKV (that only adapted up to RWKV-6 into vision) to a RWKV-7 based implementation, but since them the architeture in this repo has diverged significantly from the original Vision-RWKV. Superpixel tokenization, graph-based spatial mixing, perceptual color spaces, and Hilbert-ordered recurrence are some of the features that make this a distinct approach to efficient vision processing.

> **DISCLAIMER 2:** All the ideas behind what to do for this architeture is mine, but AI is still used in this project mainly in those distinct tasks:
> 1- Commit messages writing and automatic commit splitting (sometimes I make many changes and I use `omp commit` to figure out which file becomes which commit)
> 2- Batch code writing for chores. Sometimes I make a change that impacts many files and I use AI to write the code for all those files at once, like when I needed to change the test suite or when I refactored the architecture into more atomic modules (was single file before). This is a huge time saver, but it also means that some of the code in this repo was not written by me directly, but by an AI assistant (running mainly on Oh My Pi, but I am testing OpenCode now too). I try to review and understand all the code that is generated by AI, and many times I even need to rewrite some of the code generated, but I want to be transparent about this aspect of the project.
> 3- Sumarization of test training results that get into the README. I use AI to help me write the text that describes the the training tests results, but all the analysis and insights are my own.
